...
const projects: Project[] = [
  { name: 'InvestNest', year: '2025', description: 'A full-stack personal finance application built by a six-person team. I developed the Node.js and Express backend supporting a React and TypeScript frontend with PostgreSQL.', technologies: ['React', 'TypeScript', 'Node.js', 'Express', 'PostgreSQL'], image: '/assets/investnest-logo.png' },
  { name: 'Paceify', year: '2025', description: "A 24-hour Pitt SteelHacks XII project that generates running playlists by matching a user's pace and cadence to song BPM, with Spotify OAuth 2.0 and Web API integration.", technologies: ['React', 'Node.js', 'Spotify Web API'], image: '/assets/paceify.png' },
  { name: 'Unreal Engine FPS', year: '2024', description: 'A first-person shooter where players locate and destroy targets within a time limit. Gameplay mechanics, UI, and asset integration were built with Unreal Engine Blueprint.', technologies: ['Unreal Engine 5', 'Blueprint'], image: '/assets/unrealgame.png', links: [{ label: 'Download', href: 'https://drive.google.com/file/d/1gOoIOaqV30_wDXjOftadYH0hWQy_C9AC/view?usp=sharing' }, { label: 'Demo', href: 'https://youtu.be/V_ag0Uyeo6c?feature=shared' }] },
]
...
function Navbar() {
  ...
  const navItems = [{ label: 'About', href: '/#about' }, { label: 'Experience', href: '/#experience' }, { label: 'Projects', href: '/projects' }]
  return <header className="site-header"><div className="nav-shell"><Link to="/" className="wordmark" onClick={close}>Sean Morisoli</Link><button className="menu-button" aria-label={open ? 'Close navigation' : 'Open navigation'} aria-expanded={open} onClick={() => setOpen(!open)}>{open ? <X size={21} /> : <Menu size={21} />}</button><nav className={`nav-links ${open ? 'is-open' : ''}`} aria-label="Primary navigation">{navItems.map((item) => item.href.startsWith('/#') ? <a key={item.label} href={item.href} onClick={close}>{item.label}</a> : <Link key={item.label} to={item.href} onClick={close} className={location.pathname === item.href ? 'active' : ''}>{item.label}</Link>)}<ExternalLink href="https://github.com/seanmorisoli"><img className="social-image" src="/assets/github.png" alt="GitHub" /></ExternalLink><ExternalLink href="https://www.linkedin.com/in/sean-morisoli"><img className="social-image" src="/assets/linkedin.png" alt="LinkedIn" /></ExternalLink><a href="/assets/morisoli-resume.pdf" target="_blank" rel="noreferrer" className="resume-link">Resume</a></nav></div></header>
}
